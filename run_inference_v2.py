...
def load_dinepa(epoch):
    ...
    r_path = "../../rtcalumby/adam/luciano/LifeCLEFPlant2022/logs/PlantNet300k_exp80/jepa-ep{}.pth.tar".format(
        epoch
    )
...
def load_vanilla_vjepa(epoch):
    tag = "vjepa2"
    model_path = "./logs/" + f"{tag}" + f"-ep{epoch}.pth.tar"
...
for year in years:
    ...
    torch.save(
        predictions,
        "predictions/predictions_{}_{}.pth".format(model_type, task + "test" + year),
    )
